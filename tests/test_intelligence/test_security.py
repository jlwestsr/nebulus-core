...
class TestValidateTableName:
    ...
    def test_empty_table_name(self) -> None:
        """Empty table name raises ValidationError."""
        with pytest.raises(ValidationError, match="cannot be empty"):
            validate_table_name("")

    def test_table_name_too_long(self) -> None:
        """Excessively long table name raises ValidationError."""
        with pytest.raises(ValidationError, match="too long"):
            validate_table_name("a" * 200)

    def test_invalid_characters(self) -> None:
        """Table names with SQL injection or spaces are rejected."""
    ...
    def test_reserved_keywords(self) -> None:
        """SQL reserved keywords are rejected as table names."""
...
class TestValidateColumnName:
    ...
    def test_valid_column_names(self) -> None:
        """Valid column names pass validation unchanged."""
        assert validate_column_name("id") == "id"
        assert validate_column_name("user_name") == "user_name"

    def test_empty_column_name(self) -> None:
        """Empty column name raises ValidationError."""
        with pytest.raises(ValidationError, match="cannot be empty"):
            validate_column_name("")

    def test_invalid_column_characters(self) -> None:
        """Column names with SQL injection characters are rejected."""
...
class TestSanitizeTableName:
    ...
    def test_basic_sanitization(self) -> None:
        """Sanitization normalizes spaces and hyphens to underscores."""
...
class TestQuoteIdentifier:
    ...
    def test_double_quote_escaping(self) -> None:
        """Double quotes within identifiers are escaped by doubling."""
...
class TestValidateSqlQuery:
    ...
    def test_valid_select_queries(self) -> None:
        """Valid SELECT queries pass validation unchanged."""
    ...
    def test_non_select_blocked(self) -> None:
        """Non-SELECT statements are blocked in read-only mode."""
    ...
    def test_dangerous_keywords_blocked(self) -> None:
        """Queries containing dangerous keywords like DROP are blocked."""
    ...
    def test_comment_injection_blocked(self) -> None:
        """SQL comment injection attempts are blocked."""
    ...
    def test_allow_write_mode(self) -> None:
        """Write mode permits INSERT and UPDATE statements."""
